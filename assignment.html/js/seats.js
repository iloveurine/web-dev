...
const pricing = { VIP: 60, General: 30 };
...
function updateSeatSummary() {
  ...
  if (!summary) return;
  if (!selectedSeat) {
    summary.innerHTML = '<p>No seat chosen yet. Click a seat to reserve it.</p>';
    return;
  }
  summary.innerHTML = `
    <p><strong>Seat:</strong> ${selectedSeat.seatNumber}</p>
    <p><strong>Section:</strong> ${selectedSeat.ticketType}</p>
    <p><strong>Price:</strong> RM${selectedSeat.ticketPrice}</p>
  `;
}
...
function confirmBooking(event) {
  ...
  if (!selectedSeat) {
    alert('Please pick a seat before reserving.');
    return;
  }
  ...
  alert('Seat reserved! Check your dashboard for the details.');
